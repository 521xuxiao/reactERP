...
class WarehouseAllotting extends Component{
                    ...
                    <div className="modal1">
                        ...
                        <ul className="salesInvoiceUl salesInvoiceUl1">
                            ...
                            <li className="salesInvoiceLi">
                                ...
                                <div className="salesInvoiceDiv1">
                                    <Select className="input3" style={{width: '100%'}} value={this.state.outboundWarehouseId}
                                            onChange={(e)=>{this.setState({outboundWarehouseId: e})}}
                            ...
                            <li className="salesInvoiceLi">
                                ...
                                <div className="salesInvoiceDiv1">
                                    <Select className="input3" style={{width: '100%'}} value={this.state.inboundWarehouseId}
                                            onChange={(e)=>{this.setState({inboundWarehouseId: e})}}
                                            filterOption={(input, option) =>
                                                option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                                            } showSearch disabled={this.state.flag==3?true:false}>
                                        {this.state.outboundWarehouseIdList.map((item)=>{
                        ...
                        <div className="placeChangeTable">
                            <Table pagination={false} dataSource={this.state.modalData} locale={{emptyText: '暂无数据'}} scroll={{x: 2000}}>
                                ...
                                <Column width={200} title="包装版面" dataIndex="packagingLayout" key="packagingLayout" align="center"
                                        disabled={this.state.flag==3?true:false}
                                        render={(text, record)=>(
                                    <>
    ...
    add() {
        ...
        this.setState({
            ...
            deliveryDate,
            modalData: [
                {id: new Date().getTime(), key: new Date().getTime(), bucketNum: 0, boxNum: 0, materialCode: ''}
            ],
            outboundWarehouseId: "",
            inboundWarehouseId: "",
            keeperEmpno: "",
    ...
    queryId(row) {
        Axios.post("/self/erp/productManage/queryTransferSheetById", {id: row.id}).then((res)=>{
            if(res.data.success) {
                let obj = res.data.data.transferSheets;
                let modalData = [];
                obj.transferSheetItems.forEach((item, index)=>{
                    modalData.push({
                        key: item.id,
                        sourceSheetId: '',  // 生成填写
                        boxNum: item.boxNum,
                ...
                this.setState({
                    code: obj.code,
                    outboundWarehouseId: obj.outboundWarehouseId,
                    inboundWarehouseId: obj.inboundWarehouseId,
                    keeperEmpno: obj.keeperEmpno,
                    supplier: obj.supplier,
                    logisticsNo: obj.logisticsNo,
                    checkLeader: obj.checkLeader,
                    deliveryDate: obj.transferDate,
                    check: obj.auditName,
    ...
    deletes(row) {
        ...
        confirm({
            ...
            onOk() {
                ...
                Axios.post('/self/erp/productManage/deleteTransferSheet', params).then((res)=>{
                    if(res.data.success) {
    ...
    isCheck(row, auditStatus) {
        ...
        confirm({
            ...
            onOk() {
                ...
                Axios.post('/self/erp/productManage/auditTransferSheet', params).then((res)=>{
                    if(res.data.success) {
                        message.success("成功");
                        that.initData(that.state.currentPage);
                    }else{
    ...
    addModalData() {
        ...
        this.state.modalData.push({id: str, key: str, bucketNum: 0, boxNum: 0, materialCode: ''});
        this.setState({
            modalData: [...this.state.modalData]
        })
    }
