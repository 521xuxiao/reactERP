...
class ProductManagement extends Component{
    constructor(props){
        ...
        this.state = {
            ...
            host:"",
            uploadImgList: [],
            rawMaterialVarietyList: [], rawMaterialVariety: ''
        }
    }
    render() {
                                ...
                                <div className="modal1">
                                    <div className="div3">
                                        ...
                                        <li className="li1">
                                            ...
                                            <Select className={"input3"} allowClear onChange={(e)=>{this.setState({rawMaterialVariety: e})}} value={this.state.rawMaterialVariety}
                                                    disabled={this.state.flag==3?true:false}>
                                                {
                                                    this.state.rawMaterialVarietyList.map((item, index)=>{
                                                        return(
                                                            <Option value={item.valueId} key={index}>{item.value}</Option>
                                                        )
                                                    })
    ...
    changes(row) {
        this.setState({
            ...
            rawMaterialVariety: row.varietyId
        });
        // 获取图片作为回显
       this.showImg(row.id);
    }
    ...
    handleOk() {
        ...
        let params = {
            ...
            picture2,
            picture3,
            varietyId: this.state.rawMaterialVariety
        };
        ...
        Axios.post('self/erp/baseinfo/queryDictTypeAndValue').then((res)=>{
            ...
            if(res.data.success) {
                this.setState({
                    ...
                    rawMaterialVarietyList: res.data.data.rawMaterialVariety
                })
            }else{
                this.setState({
